...
namespace SudokuGameWPF.Model
{
    internal class GameModel
    {
        #region . Variables .

        private CellClass[,] cells;                            // Array of cells for the puzzle that is playing.
        private List<CellClass>[] regionList;                  // Array of cells arranged by region.
        ...
        internal CellClass this[Int32 col, Int32 row]
        {
            get
            {
                if ((cells != null) && (Common.IsValidIndex(col, row)))    // If we have cells and the inputs are valid.
                    return cells[col, row];                                // Then return the specified cell.
        ...
        internal void ComputeNote(Int32 col, Int32 row)
        {
            if (Common.IsValidIndex(col, row))              // Are the inputs valid?
                GenerateNote(cells[col, row]);             // Yes, then generate the note.
        }
        ...
        {
            if (cells != null)                                         // Do we have a game stored?
            {                                                           // Yes.
        ...
        {
            if (cells != null)                                         // Do we have a game saved?
        ...
        {
            if (Common.IsValidIndex(index) && (regionList != null))    // If the input is valid and the region list is not null.
                return regionList[index];                              // Return the region requested.
            return null;                                                // Otherwise, return null.
        }
        ...
        {
            regionList = new List<CellClass>[9];                   // Initialize the array with 9 elements.
            for (Int32 i = 0; i < 9; i++)                           // Loop through the array.
                regionList[i] = new List<CellClass>();             // Initialize each element with a list.
        }

        private void ConvertToList()
        {
            CellList = new List<CellClass>(cells.Length);          // Initialize the list.
            for (Int32 col = 0; col < 9; col++)                     // Loop through the columns
                for (Int32 row = 0; row < 9; row++)                 // Loop through the rows
                    AddCell(cells[col, row]);                      // Add each cell to the lists
        }
        ...
        private void AddCell(CellClass cell)
        {
            ...
            {                                                       // Yes.
                ...
                regionList[cell.Region].Add(cell);                 // Add the cell the corresponding region list.
            }
            else
        ...
        private void GenerateNote(CellClass cell)
        {
            ...
            {                                                           // No, the figure out the notes
                ...
                {
                    ProcessNote(cell, cells[cell.Col, i]);             // Check the column the cell is in.
                    ProcessNote(cell, cells[i, cell.Row]);             // Check the row the cell is in.
                }
                foreach (CellClass item in regionList[cell.Region])    // Now loop through the region the cell belongs to.
